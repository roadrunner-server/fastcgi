...
use fastcgi::Role;
use fastcgi::{RequestResult, Request};
...
pub trait TestCase {
    fn get_input() -> Mock;
    fn get_output() -> Mock;
    fn processor<W: Write>(request: Arc<Request<W>>) -> RequestResult;
}
...
impl TestCase for TestParamsInOut {
    fn get_input() -> Mock {
        Builder::new()
            ...
            .read(&create_record(
                ...
                0x01,
                0x06,
                b"\x0B\x02SERVER_PORT80\x04\x03TESTYES\x06\x03NOUTF8NO\xF0",
            ))
    ...
    fn processor<W: Write>(request: Arc<Request<W>>) -> RequestResult {
        ...
        request.get_stdout().write(b"TEST1234").unwrap();
...
impl TestCase for TestRoleFilter {
    fn get_input() -> Mock {
        Builder::new()
            ...
            .read(&create_record(
                ...
                0x01,
                0x00,
                b"\x12\x0AFCGI_DATA_LAST_MOD1595418756\x10\x02FCGI_DATA_LENGTH12",
            ))
            .read(&create_record(RecordType::Params, 0x01, 0x00, &[]))
            .read(&create_record(
                RecordType::Data,
    ...
    fn processor<W: Write>(request: Arc<Request<W>>) -> RequestResult {
        ...
        assert_eq!(
            String::from_utf8(last_mod.unwrap().to_vec()).unwrap(),
            "1595418756"
        );
        ...
        assert_eq!(
            String::from_utf8(data_length.unwrap().to_vec()).unwrap(),
            "12"
        );
...
impl TestCase for TestAbortRequest {
    ...
    fn processor<W: Write>(_request: Arc<Request<W>>) -> RequestResult {
...
impl TestCase for TestAbortContinue {
    fn get_input() -> Mock {
        Builder::new()
            ...
            /*Request 0*/
            .read(&create_record(RecordType::Params, 0x00, 0x00, &[]))
            /*Request 1*/
            .read(&create_record(
                RecordType::Params,
                0x01,
            ...
            /*Request 0*/
            .read(&create_record(RecordType::StdIn, 0x00, 0x03, &[]))
            .build()
    }
    ...
    fn processor<W: Write>(request: Arc<Request<W>>) -> RequestResult {
        ...
        request.get_stdout().write(b"0").unwrap();
    ...
    fn get_input() -> Mock {
        Builder::new()
            ...
            .read(&create_record(
                ...
                0x01,
                0x06,
                b"\x0B\x02SERVER_PORT80\x04\x03TESTYES",
            ))
            .read(&create_record(RecordType::Params, 0x01, 0x00, &[]))
            .read(&create_record(
                RecordType::StdIn,
...
impl TestCase for TestUnknownRequestType {
    ...
    fn get_output() -> Mock {
        Builder::new()
            ...
            .write(&[
                14u8, 1, b'F', b'C', b'G', b'I', b'_', b'M', b'A', b'X', b'_', b'C', b'O', b'N',
                b'N', b'S', b'5',
            ])
            .build()
    }
...
impl TestCase for TestGetValues {
    fn get_input() -> Mock {
        Builder::new()
            .read(&create_record(
                ...
                0x01,
                0x00,
                b"\x0e\x00FCGI_MAX_CONNS\x0d\x00FCGI_MAX_REQS\x0f\x00FCGI_MPXS_CONNS",
            ))
            .build()
    }
    ...
    fn get_output() -> Mock {
        Builder::new()
            ...
            .write(&[
                13u8, 2, b'F', b'C', b'G', b'I', b'_', b'M', b'A', b'X', b'_', b'R', b'E', b'Q',
                b'S', b'1', b'0',
            ])
            .write(&[
                15u8, 1, b'F', b'C', b'G', b'I', b'_', b'M', b'P', b'X', b'S', b'_', b'C', b'O',
                b'N', b'N', b'S', b'1',
            ])
            .build()
    }
    ...
    fn processor<W: Write>(_request: Arc<Request<W>>) -> RequestResult {
        unreachable!(
            "This should never run because a get_value request should never run the processor."
        );
    }
}
...
impl TestCase for TestKeepConnection {
    ...
    fn processor<W: std::io::Write>(request: Arc<Request<W>>) -> RequestResult {
        let idx = request.get_str_param("IDX").unwrap();

        request.get_stdout().write(idx.as_bytes()).unwrap();
        request
            .get_stderr()
            .write(&[b'X', idx.as_bytes()[0] - b'1' + b'A'])
            .unwrap();
