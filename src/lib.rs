...
impl<S: Copy + TryFrom<u8, Error = TypeError>, T: Copy + TryFrom<u8, Error = TypeError>> TryFrom<u8>
    for Category<S, T>
{
    type Error = TypeError;
    fn try_from(value: u8) -> Result<Self, Self::Error> {
...
impl<S: std::convert::Into<u8> + Copy, T: std::convert::Into<u8> + Copy> From<Category<S, T>>
    for u8
{
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            Error::UnknownRecordType(request_id, type_id) => write!(
                f,
                "Unkown record type {} in request {} received",
                type_id, request_id
            ),
...
impl Record {
    fn new<R: Read>(rd: &mut R) -> Result<Self, Error> {
        let mut header_buffer = [0; RECORD_HEADER_SIZE];

        rd.read_exact(&mut header_buffer)?;
        ...
        let record_type =
            RequestType::try_from(byteorder::ReadBytesExt::read_u8(&mut header_slice).unwrap());
        let request_id = byteorder::ReadBytesExt::read_u16::<BigEndian>(&mut header_slice)?;
        let content_length =
            byteorder::ReadBytesExt::read_u16::<BigEndian>(&mut header_slice).unwrap() as usize;
        ...
        rd.read_exact(&mut content)?;

        // If there is some padding at the end of the record. Discard it.
        if padding_length > 0 {
            std::io::copy(&mut rd.take(padding_length), &mut std::io::sink())?;
        }

        trace!(
            "FastCGI: In record {{T:{:?}, ID: {}, L:{}}}",
            record_type,
            request_id,
            RECORD_HEADER_SIZE + content.len() + padding_length as usize
        );
...
impl<W: Write> Request<W> {
    fn new(record: &Record, writer: Arc<Mutex<W>>) -> Result<Self, Error> {
        ...
        if let Category::Std(StdReqType::BeginRequest) = record.record_type {
            if let Some(role) = Role::from_number(
                byteorder::ReadBytesExt::read_u16::<BigEndian>(&mut content).unwrap(),
            ) {
                //We're reading from am memory buffer. So there is something deeply wrong if this fails.
                let keep_connection = (byteorder::ReadBytesExt::read_u8(&mut content)?
                    & FCGI_KEEP_CONN)
                    == FCGI_KEEP_CONN;
    ...
    fn add_nv_pairs(
        params: &mut HashMap<String, Vec<u8>>,
        src: &[u8],
        lowercase_keys: bool,
    ) -> Result<(), std::io::Error> {
        ...
        while !src_slice.is_empty() {
            ...
            trace!(
                "FastCGI: NV-Pair[\"{}\"]=\"{}\"",
                key,
                String::from_utf8_lossy(&value_buffer)
            );
    ...
    ///
    /// ```rust
    /// # use fastcgi::{Requests, RequestResult};
    /// # fn main() {
    /// # let mut requests = Requests::new(std::io::empty(), std::io::sink(), 1, 1);
    /// # if let Some(request) = requests.read_next_request().expect("Request could not be constructed.") {
    ...
    pub fn params_iter(&self) -> Option<Box<ParamsIterator>> {
        if self.params_done {
            Some(Box::new(
                self.params.iter().map(|v| (v.0.as_str(), &v.1[..])),
            ))
        } else {
            None
        }
    ...
    pub fn str_params_iter(&self) -> Option<Box<StrParamsIterator>> {
        if self.params_done {
            Some(Box::new(
                self.params
                    .iter()
                    .map(|v| (v.0.as_str(), std::str::from_utf8(v.1).ok())),
            ))
        } else {
            None
        }
            ...
            match record_type {
                ...
                StdReqType::Params => {
                    if record.content.is_empty() {
                        ...
                        if self.params_done {
                            warn!("FastCGI: Protocol error. Params received after params stream was marked as done.");
                        }
    ...
    ///
    ///   assert!(stdout.write(b"Hello World").is_ok());
    ///
    ///   RequestResult::Complete(0)
    /// });
    /// # } }
    ...
    ///
    ///   assert!(stderr.write(b"Hello World").is_ok());
    ///
    ///   RequestResult::Complete(0)
    /// });
    /// # } }
    ...
    ///
    /// while let Some(request) = requests.read_next_request().expect("Request could not be constructed.") {
    ///   request.process(|request| {
    ///
    ///     // Process request
    ///
    ...
    /// # }
    /// ```
    pub fn process<C: FnOnce(Arc<Self>) -> RequestResult>(self, callback: C) -> Result<(), Error> {
        let rc_self = Arc::from(self);

        let result = callback(rc_self.clone());

        if let Ok(this) = Arc::try_unwrap(rc_self) {
            this.get_stdout().close()?;
            this.get_stderr().close()?;

            this.orw.write_finish(result)?;
        } else {
...
pub struct Requests<R: Read, W: Write> {
    reader: R,
    writer: Arc<Mutex<W>>,
...
impl<R: Read, W: Write> Requests<R, W> {
    ...
    fn process_sys(&self, record: Record) -> Result<Option<RequestId>, Error> {
        ...
        if let Category::Sys(record_type) = record.record_type {
            ...
            let result = match record_type {
                SysReqType::GetValues => {
                    ...
                    output_stream
                        .write_data(Category::Sys(SysRespType::GetValuesResult), &output[..])?;

                    Ok(None)
                }
                SysReqType::AbortRequest => {
                    output_stream.write_finish(RequestResult::Complete(0))?;
    ...
    ///
    /// This function synchronously fetches FastCGI records and assembles them
    ...
    pub fn read_next_request(&mut self) -> Result<Option<Request<W>>, Error> {
        if self.close_on_next {
            ...
            loop {
                match Record::new(&mut self.reader) {
                    // Success, a new record hast to be added to its request...
                    Ok(record) => {
                        if record.is_sys_record() {
                            if let Some(canceled_request_id) = self.process_sys(record)? {
                            ...
                            let request_ready = match self.requests.entry(record.get_request_id()) {
                                ...
                                Entry::Vacant(e) => {
                                    e.insert(Request::new(&record, self.writer.clone())?);
                                    Ok(false)
                                }
                            }?;
                            ...
                            if request_ready {
                                let request =
                                    self.requests.remove(&record.get_request_id()).unwrap();
                    ...
                    Err(Error::UnknownRecordType(request_id, type_id)) => {
                        ...
                        output_stream.write_unkown_type(type_id)?;
                    }
...
impl<W: Write> Debug for Request<W> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Request {{ request_id: {}, keep_connection: {:?}, stdin: {:?}, data: {:?}, params: {{",
            self.request_id, self.keep_connection, self.stdin, self.data
        )?;
...
#[derive(Debug)]
struct OutRecordWriter<W: Write> {
    inner_stream: Arc<Mutex<W>>,
    request_id: RequestId,
}

impl<W: Write> OutRecordWriter<W> {
    ...
    fn write_data(
        ...
        trace!(
            "FastCGI: Out record {{T:{:?}, ID: {}, L:{}}}",
            record_type,
            self.request_id,
            RECORD_HEADER_SIZE + data.len()
        );
        ...
        byteorder::WriteBytesExt::write_u16::<BigEndian>(&mut message_header, self.request_id)
            .unwrap(); // Request ID
        byteorder::WriteBytesExt::write_u16::<BigEndian>(&mut message_header, data.len() as u16)
    ...
    /// request.
    fn write_finish(&self, result: RequestResult) -> Result<(), std::io::Error> {
        ...
        self.write_data(Category::Std(StdRespType::EndRequest), &end_message[..])?;

        Ok(())
    }

    /// Sends an `UnknownType` response to the web-server.
    fn write_unkown_type(&self, type_id: u8) -> Result<(), std::io::Error> {
        ...
        self.write_data(ResponseType::Sys(SysRespType::UnknownType), &ut_message[..])?;

        Ok(())
    }

    fn flush(&self) -> std::result::Result<(), std::io::Error> {
        self.inner_stream.try_lock().expect(ERR_LOCK_FAILED).flush()
    }
}
...
impl<W: Write> OutStream<W> {
    ...
    pub fn write(&mut self, data: &[u8]) -> std::result::Result<usize, Error> {
        if self.closed {
        ...
        if data.len() < u16::max_value() as usize {
            Ok(self.orw.write_data(self.record_type, data)?)
        } else {
            ...
            for offset in (0..data.len()).step_by(JUNK_SIZE) {
                self.orw.write_data(
                    self.record_type,
                    &data[offset..(offset + JUNK_SIZE).min(data.len())],
                )?;
            }
    ...
    pub fn flush(&self) -> std::result::Result<(), std::io::Error> {
        self.orw.flush()
    }
